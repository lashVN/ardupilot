...
void AP_ExternalAHRS_VectorNav::wait_register_response() {
...
void AP_ExternalAHRS_VectorNav::initialize() {
    ...
    hal.util->snprintf(message_to_send, sizeof(message_to_send), "$VNWRG,6,0\0");
    wait_register_response();

    // Detect version
    // Read Model Number Register, ID 1
    hal.util->snprintf(message_to_send, sizeof(message_to_send), "$VNRRG,1\0");
    wait_register_response();
